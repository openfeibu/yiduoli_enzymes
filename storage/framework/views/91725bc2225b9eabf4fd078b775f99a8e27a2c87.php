<link rel="stylesheet" href="<?php echo theme_asset('css/30th.css'); ?>">   
<div class="banner">
    <img class="pcImg" src="<?php echo theme_asset('images/30/banner.png'); ?>" width="100%" alt="" />
    <div class="h5Img" width="100%" alt="" style="background:url(<?php echo theme_asset('images/30/banner.png'); ?>) no-repeat center / cover"></div>
</div>
<!-- 直播-->
<div class="live">
	<div class=" tip-title2 wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".3s">
           <img  class="" src="<?php echo theme_asset('images/30/live.png'); ?>" alt="">
	</div>
	    <div class="container w1200 live-con" style="max-width:1320px !important">	
	
			<div class=" live-left col-xs-12 col-sm-12 wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".3s"  >
				<div class="live-img-item " style="display:block;">
					<a target="_black" href="https://wx.vzan.com/live/tvchat-674786388?v=1616488887521"><img class="transition" src="<?php echo theme_asset('images/30/liveimg1.jpg'); ?>" alt="" /></a>
				</div>
				<div class="live-img-item">
					<a target="_black" href="https://wx.vzan.com/live/tvchat-526384769?v=1616488728261"><img class="transition" src="<?php echo theme_asset('images/30/liveimg2.jpg'); ?>" alt="" /></a>
				</div>
				<!--<div class="live-img-item">
					<a  target="_black" href="https://www.pailixiang.com/album_ia1180856897.html"><img class="transition" src="<?php echo theme_asset('images/30/liveimg3.jpg'); ?>" alt="" /></a>
				</div>-->

			</div>
			<div class="live-right col-xs-12  col-sm-12" style="padding-right:0;">
				<div class="live-test-item active  wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".3s">
					<h3>溢多利成立三十周年庆典活动直播</h3>
					<p>
						<span>地点：</span>
						广东溢多利总部
					</p>
					<p>
						<span>时间：</span>
						2021年3月30日 08:30-11:30
					</p>
				</div>
				<div class="live-test-item  wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".4s">
					<h3>粤港澳大湾区无抗饲料国际论坛直播</h3>
					<p>
						<span>地点：</span>
						珠海长隆横琴湾酒店会议厅
					</p>
					<p>
						<span>时间：</span>
						2021年3月30日 13:30-18:30 
					</p>
				</div>
				<!--<div class="live-test-item  wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".5s">
					<h3>三十周年庆典活动与高端论坛<br>现场图片云直播</h3>
					<p>
						云呈现活动现场的精彩瞬间
					</p>
					
				</div>-->
			</div>
		   
		
    </div>
</div>


<!--祝福-->
<div class="greetings">
	<div class=" tip-title2 wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".3s">
           <img  class="" src="<?php echo theme_asset('images/30/khzf.png'); ?>" alt="">
	</div>
	<div class="greetings-con container w1200">
		<div id="greetingsPlayer"></div>
	</div>
</div>
<!-- 关于我们 -->
<div class="about">
	<div class=" tip-title2 wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".3s">
           <img  class="" src="<?php echo theme_asset('images/30/qyxcp.png'); ?>" alt="">
	</div>
    <div class="container qy-con w1200" style="max-width:1382px !important">
	
	 <div class="page-news-video about-left col-lg-6 col-md-6 col-sm-12 wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".3s"  vid="ad514bee27f277d57283a05c13846c1e_a" des="溢多利2021新版官方宣传片">

            <img class="transition" src="<?php echo theme_asset('images/30/video.png'); ?>" alt="" />

        </div>
		<div class="about-right col-lg-6 col-md-6 col-sm-12" style="padding-right:0;">
		
            <div class="about-con  wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".5s">
				<div class="t">三十而“利”正当时 <br> 扬帆奋进开新局！</div>
                <p>2021年，溢多利正式迎来公司成立30周年的特殊时刻。1991年，在珠海经济特区，溢多利酶制剂有限公司（溢多利的前身）诞生，中国饲用酶制剂工业就此起航。经过30年跨越式发展，溢多利已经成为我国生物酶制剂行业龙头企业，中国动物营养与健康领域领军企业，全球极具竞争力的甾体激素医药企业。2021年是国家“十四五”规划的开局之年，在更高的起点，溢多利作为粤港澳大湾区的生物技术领军企业，将持续进行生物技术的革新，立足生物酶制剂、生物医药、动物营养与健康领域，结合政策与行业发展趋势等推出更具有前瞻性的产品，以卓越的产品与服务满足行业需求，为建设人类绿色、健康的美好生活而不懈努力！
                </p>
         
            </div>
            <div class="more  wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".6s">查看全部</div>
        </div>
       
		
    </div>
</div>
<div class="product developmentCourse">

        <div class=" tip-title2 wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".3s">
           <img  class="" src="<?php echo theme_asset('images/30/fzlc.png'); ?>" alt="">
        </div>

		<div class="developmentCourse-con clearfix">
			<div class="container w1200  wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".5s" style="max-width:1416px !important">
					<div class="swiper-container-wrapper ">
						
						
					  <!-- Slider main container -->
					  
					  <div class="swiper-container  gallery-lc col-lg-10 col-md-10 col-xs-9">
						<!-- Additional required wrapper -->
						<div class="swiper-wrapper">
						  <!-- Slides -->
							 <div class="swiper-slide ">
								
								<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/1991.jpg'); ?>" alt="" />
								 </div>
								  <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
									<div class="time"></div>
									<div class="des">1991年，溢多利创立，标志中国饲用酶制剂工业起步</div>
								</div>	
							 </div>
							 <div class="swiper-slide ">
							 <div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/1997.jpg'); ?>" alt="" />
								 </div>
								 <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">1997年，溢多利珠海生产基地一期工程竣工。</div>
								</div>
							</div>
							<div class="swiper-slide ">
								<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/1998.jpg'); ?>" alt="" />
								 </div>
								 <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">1998年，溢多利被认定为广东省高新技术企业。</div>
								 
							   </div>
							</div>
							<div class="swiper-slide ">
								<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/1999.jpg'); ?>" alt="" />
								 </div>
								 <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">1999年，溢多利在中国农业大学等多所高校捐资设立溢多利教育基金。</div>
								</div>
							</div>
							<div class="swiper-slide ">
								<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/2001.jpg'); ?>" alt="" />
								 </div>
								 <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">2001年，溢多利完成股份改制，为迈向资本市场奠定基础。</div>
								  
							   </div>
							</div>
							<div class="swiper-slide ">
								<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/2003.jpg'); ?>" alt="" />
								 </div>
								 <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">2003年，溢多利组建成立我国首个饲添省级工程中心；溢多利起草的《饲料用酶制剂通则》由农业部颁布施行。</div>
								  
							   </div>
							</div>
							<div class="swiper-slide ">
								<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/2005.jpg'); ?>" alt="" />
								 </div>
								 <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">2005年，溢多利被评为“全国三十强饲料企业”。</div>
								</div>
								
							</div>
							<div class="swiper-slide ">
								<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/2008.jpg'); ?>" alt="" />
								 </div>
								 <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">2008年，溢多利被认定为行业首批国家高新技术企业。</div>
								  
							   </div>
							</div>
							<div class="swiper-slide ">
								<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/2014.jpg'); ?>" alt="" />
								 </div>
								 <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">2014年，溢多利在深交所上市，成为中国生物酶制剂行业首家上市企业；并购湖南鸿鹰，拓宽酶制剂产品应用领域。</div>
								   
								</div>
							
							</div>
							<div class="swiper-slide ">
								<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/2015.jpg'); ?>" alt="" />
								 </div>
								 <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">2015年，溢多利博士后科研工作站获批成立；并购湖南新合新、河南利华，进入生物医药领域，步入跨越式发展快车道。</div>
								</div>
								
							</div>
							<div class="swiper-slide ">
							<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/2016.jpg'); ?>" alt="" />
								 </div>
								 <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">2016年，溢多利总部研发与办公大楼落成；新产学研示范基地建成并投入使用。</div>
								</div>
							</div>
							<div class="swiper-slide ">
								<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/2017.jpg'); ?>" alt="" />
								 </div>
								 <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">2017年，公司启动“溢多利生物医药产业园”投资建设，打造全球一流产业基地。</div>
								</div>
								
							</div>
							<div class="swiper-slide ">
								<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/2018.jpg'); ?>" alt="" />
								 </div>
								 <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">2018年，溢多利荣获“国家认定企业技术中心”；并购长沙世唯，布局饲用替抗市场。</div>
								   
								</div>
							</div>
							<div class="swiper-slide ">
								<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/2020.jpg'); ?>" alt="" />
								 </div>
								<div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">2020年，溢多利推出饲用替抗产品，迎接饲料无抗新时代；溢多利“抗疫情，保生产”，全力保障客户产品需求。</div>
								   
								  
								</div>
								
							</div>
							<div class="swiper-slide ">
								<div class="img col-lg-6 col-md-6 col-sm-12 col-xs-12">
									<img  class="" src="<?php echo theme_asset('images/30/2021.jpg'); ?>" alt="" />
								 </div>
								 <div class="test col-lg-5 col-md-5 col-sm-12 col-xs-12">
								   <div class="time"></div>
								   <div class="des">2021年，溢多利成立30周年，开启新征程。</div>
								   
								</div>
								
							</div>
							
						</div>
						<!-- Add Arrows
						<div class="thumbs-next"></div>
						<div class="thumbs-prev"></div> -->
					  </div>
					  <!-- Slider thumbnail container -->
					 <div class="col-lg-2 col-md-2 col-xs-3 timebox" style="position:relative">
							<div class="swiper-button-prev thumbs-prev"></div>
							<div class="swiper-container gallery-thumbs ">
								<!-- Additional required wrapper -->
								<div class="swiper-wrapper  gallery-thumbs-b">
								  <!-- Slides -->
									<div class="swiper-slide ">
										<div class="time">1991</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">1997</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">1998</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">1999</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">2001</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">2003</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">2005</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">2008</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">2014</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">2015</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">2016</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">2017</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">2018</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">2020</div>									
									</div>
									<div class="swiper-slide ">
										<div class="time">2021</div>									
									</div>
									
								</div>
								
								
							</div>
							<div class="swiper-button-next thumbs-next"></div>
						</div>
					</div>
			</div>
		</div>
</div>
<div class="photoE wkMt" >
    <div class="">
        <!--  <div class="con-title tip-title2 wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".3s">
            <h1><b>不忘初心"溢"往无前</b>图片展</h1>
        </div> -->
		<div class=" tip-title2 wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".3s">
           <img  class="" src="<?php echo theme_asset('images/30/bwcx.png'); ?>" alt="">
        </div>
		
        <div class="photoE-con clearfix  wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".7s">
				<div class="swiper-container-wrapper">
					  <!-- Slider main container -->
					  <div class="swiper-container gallery-top">
						<!-- Additional required wrapper -->
						<div class="swiper-wrapper">
						  <!-- Slides -->
						 
						</div>
						
						<div class="swiper-button-prev photo-prev"></div>
						<div class="swiper-button-next photo-next"></div>
						<div class="label"><img src="<?php echo theme_asset('images/30/tpz-icon.png'); ?>"></div></div>
					  </div>
					
				</div>

			
        </div>
    </div>
</div>
<div class=" vision">
    <div class="container w1200">
        <div class=" tip-title2 wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".3s">
           <img  class="" src="<?php echo theme_asset('images/30/qywh.png'); ?>" alt="">
        </div>
        <div class="vision-con clearfix">
			<div class="vision-item col-lg-4 col-md-4 col-sm-12 col-xs-12 wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".5s">
				   
					
						 <a class="clearfix" target="_black" href="http://www.yiduoli.com/about/culture">
							<div class="img" >
							  <img  class="" src="<?php echo theme_asset('images/30/culture1.png'); ?>" alt="">
							</div>
							<div class="t" >
							  企业愿景
							</div>
							<p>成为世界领先的生物技术企业 </p>
						</a>
					
					
			</div>
			 <div class="vision-item col-lg-4 col-md-4 col-sm-12 col-xs-12 wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".5s">
		
						 <a class="clearfix" target="_black" href="http://www.yiduoli.com/about/culture">
							<div class="img" >
							  <img  class="" src="<?php echo theme_asset('images/30/culture2.png'); ?>" alt="">
							</div>
						   <div class="t" >
							  企业使命
							</div>
							<p>致力于生物技术的研究与应用，以卓越的产品与服务满足客户需求，不断提升人类生活品质 </p>
						</a>
				
					
			</div>
			<div class="vision-item col-lg-4 col-md-4 col-sm-12 col-xs-12 wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".5s">
				   
					
						 <a class="clearfix" target="_black" href="http://www.yiduoli.com/about/culture">
							<div class="img" >
							  <img  class="" src="<?php echo theme_asset('images/30/culture3.png'); ?>" alt="">
							</div>
							 <div class="t" >
							  核心价值观
							</div>
							<p> 客户为先  科技为本 诚信务实  和谐共赢</p>
						</a>
			
					
			</div>
        </div>
    </div>
</div>

<style>

</style>
<div class="rcICon">
	庆典日程
</div>
<div class="rc-con" > 
	<div class="rc-bg">
		<div class="rc-table">
			<div class="rc-close"></div>
			<div class="rc-table-t">“溢多利生物产业发展与行业高端论坛暨成立三十周年庆典”活动日程</div>
			<div class="container w1200">
				 <table style="width:100%;" border="1"  cellpadding="5" align="center">
					<tr align="center" class="tr1">
					  <th>日期</th>
					  <th>活动时间</th>
					  <th>活动内容</th>
					  <th>活动地点</th>
					</tr>
					<tr align="center">
					  <td>03.29</td>
					  <td>全天</td>
					 
					  <td align="left" style="padding-left:5px">嘉宾报到</td>
					  <td>横琴湾酒店大堂</td>
					</tr>
		
					<tr align="center">
					 <td  rowspan="8" >03.30</td>
					  <td>08:30-11:30</td>
					  <td align="left" style="padding-left:5px">“溢多利生物产业发展与行业高端论坛暨成立三十周年庆典”开幕仪式</td>
					  <td>溢多利公司</td>
					</tr>
					<tr align="center">
					  <td>11:30</td>
					  <td align="left" style="padding-left:5px">统一乘车前往横琴湾酒店。</td>
					  <td>溢多利公司</td>
					</tr>
				
					<tr align="center">
					  <td  colspan="3" >溢多利生物产业发展与行业高端论坛</td>
					  
					</tr>
					<tr align="center">
					  <td>13:30-18:30</td>
					  <td align="left" style="padding-left:5px">第一会场：<br>粤港澳大湾区无抗饲料国际论坛</td>
					  <td>横琴湾酒店水晶殿</td>
					</tr>
					<tr align="center">
					  <td>14:30-17:30</td>
					  <td align="left" style="padding-left:5px">第二会场：<br>溢多利生物医药发展高端论坛</td>
					  <td>横琴湾酒店307-309厅</td>
					</tr>
					<tr align="center">
					  <td>13:30-18:00</td>
					  <td align="left" style="padding-left:5px">第三会场：生物酶技术在工业领域的应用高端论坛</td>
					  <td>横琴湾酒店301-303厅</td>
					</tr>
					<tr align="center">
					  <td>16:00-17:30</td>
					  <td align="left" style="padding-left:5px">第四会场：溢多利投资者交流会</td>
					  <td>横琴湾酒店311-312厅</td>
					</tr>
					<tr align="center">
					  <td>18:30-21:00</td>
					  <td align="left" style="padding-left:5px">“溢多利生物产业发展与行业高端论坛暨成立三十周年庆典”招待晚宴</td>
					  <td>横琴湾酒店海洋厅</td>
					</tr>
					<tr align="center">
						<td>03.31</td>
					  <td>全天</td>
					  <td align="left" style="padding-left:5px">参观珠海横琴自贸区</td>
					  <td>珠海横琴</td>
					</tr>
				  </table>
				</div>
		</div>
	</div>
</div>
<link rel="stylesheet" href="<?php echo theme_asset('images/30/swiper-bundle.min.css'); ?>">     
<script src="<?php echo theme_asset('images/30/swiper.min.js'); ?>"> </script>
<script>
    $(function() {
        $("title").text('溢多利三十周年庆典直播入口')
        var mySwiper = new Swiper('.swiper-container-banner', {
            // loop: true,
            // autoplay: 6000,
            pagination: '.swiper-pagination-banner',
            paginationClickable :true
        })
		$(document).ready(function () {
		   var galleryThumbs = new Swiper(".gallery-thumbs", {
				direction: "vertical",
				spaceBetween: 0,
				slidesPerView: 5,

				freeMode: false,
				watchSlidesVisibility: true,
				watchSlidesProgress: true,
				
				
		  });
		  var galleryTop = new Swiper(".gallery-lc", {
			
			autoplay: {
				delay: 5000,
				stopOnLastSlide: false,
				disableOnInteraction: false,
				},
		
			direction: "vertical",
			spaceBetween: 0,
			
			observer:true,          //初始化子元素
			observerParents:true,    //初始化父元素
			navigation: {
				  nextEl: ".thumbs-next",
				  prevEl: ".thumbs-prev"
				},
			thumbs: {
			  swiper: galleryThumbs
			}
			
		  });
		 
		
		});
		
	
		
		
        var hiddenHeight = '';
        $(".about-right .more").on("click",function(){
            if($(this).hasClass("active")){

                $(".about-con p").css("height",hiddenHeight);
                $(this).removeClass("active").text("查看全部");
            }else{
                hiddenHeight = $(".about-con p").css("height");
                $(".about-con p").css("height","auto");
                $(this).addClass("active").text("收起信息");
            }
        })
    })
</script>
<div class="video-detail">
    <div class="video-detail-close"></div>
    <div id="video-detail-con">
        <div class="video-close"></div>
        <div id="playerBox">
            <div id='player'></div>
            <div class="des"></div>
        </div>

    </div>

</div>
<script>
    $(function() {
			var pic = [

		{

				title:"溢多利创立原址",

				des:"1991年，珠海经济特区溢多利酶制剂有限公司成立，标志着中国饲用酶制剂工业的起步。",

				url:"<?php echo theme_asset('images/30/01.jpg'); ?>"

			},

			{

				title:"溢多利创立初期生产厂房原貌",

				des:"1991年，溢多利创立初期生产厂房原貌，主要进行饲用酶制剂产品的生产。",

				url:"<?php echo theme_asset('images/30/02.jpg'); ?>"

			},

			{

				title:"溢多利第一期新厂奠基仪式",

				des:"1996年，在溢多利第一期新厂动工修建之际， 陈少美董事长为新厂奠基。",

				url:"<?php echo theme_asset('images/30/03.jpg'); ?>"

			},

			{

				title:"溢多利第一期公司全貌",

				des:"1997年，溢多利建成了具有办公、生产等综合功能的现代化工业园。",

				url:"<?php echo theme_asset('images/30/04.jpg'); ?>"

			},

			{

				title:"溢多利第一期工厂落成典礼",

				des:"1997年10月9日，溢多利举行第一期工厂落成典礼。",

				url:"<?php echo theme_asset('images/30/05.jpg'); ?>"

			},

			{

				title:"原农业部饲料用酶首个行标文件",

				des:"1999年，溢多利承担农业部“1999年第一批农业行业标准制、修订项目计划”中的饲料酶制剂行业标准通则的制订工作，于2001年底获得通过。",

				url:"<?php echo theme_asset('images/30/06.jpg'); ?>"

			},

			{

				title:"溢多利产品鉴定会",

				des:"1993年，溢多利进行“饲用复合酶制剂”产品鉴定会。",

				url:"<?php echo theme_asset('images/30/07.jpg'); ?>"

			},

			{

				title:"溢多利通过兽药GMP验收",

				des:"2003年7月25日，溢多利通过兽药GMP验收。",

				url:"<?php echo theme_asset('images/30/08.jpg'); ?>"

			},

			{

				title:"溢多利成立10周年庆典",

				des:"2001年8月8日，溢多利举行公司成立十周年庆典。",

				url:"<?php echo theme_asset('images/30/09.jpg'); ?>"

			},

			{

				title:"溢多利成立15周年庆典",

				des:"2006年10月12日，溢多利举行公司成立十五周年庆典。",

				url:"<?php echo theme_asset('images/30/10.jpg'); ?>"

			},

			{

				title:"溢多利成立20周年庆典",

				des:"2011年8月26日，溢多利举行公司成立二十周年庆典。",

				url:"<?php echo theme_asset('images/30/11.jpg'); ?>"

			},

			{

				title:"溢多利成立25周年庆典",

				des:"2016年6月24日，溢多利举行“生物产业发展与行业高端论坛珠海峰会暨溢多利成立二十五周年庆典”。",

				url:"<?php echo theme_asset('images/30/12.jpg'); ?>"

			},

			{

				title:"原全国人大副委员长许嘉璐亲临溢多利指导工作",

				des:"2000年4月21日，原全国人大副委员长许嘉璐亲临溢多利公司指导工作，与陈少美董事长等领导进行了详细、深入的交流。",

				url:"<?php echo theme_asset('images/30/13.jpg'); ?>"

			},

			{

				title:"全国政协副主席、民建中央常务副主席辜胜阻莅临溢多利调研指导工作",

				des:"2020年10月5日，全国政协副主席、民建中央常务副主席辜胜阻莅临溢多利进行调研指导工作，肯定了溢多利的高速发展及科研实力。",

				url:"<?php echo theme_asset('images/30/14.jpg'); ?>"

			},

			{

				title:"原国家科技部部长徐冠华莅临溢多利视察",

				des:"原国家科技部部长徐冠华在珠海市领导的陪同下莅临溢多利视察，对公司高速发展的现状表示肯定，同时勉励公司未来大力提升科研水平、再创佳绩。",

				url:"<?php echo theme_asset('images/30/15.jpg'); ?>"

			},

			{

				title:"农业农村部副部长于康震及其他领导亲临溢多利展位考察指导",

				des:"2014年中国饲料工业展览会，农业农村部副部长于康震等领导亲临溢多利展位并与陈少美董事长合影留念。",

				url:"<?php echo theme_asset('images/30/16.jpg'); ?>"

			},

			{

				title:"陈少美董事长陪同原国家饲料办主任王维四在溢多利考察 ",

				des:"创业早期，陈少美董事长陪同国家饲料办领导在溢多利考察，王维四主任一行对溢多利公司的各项工作给予充分肯定。",

				url:"<?php echo theme_asset('images/30/17.jpg'); ?>"

			},

			{

				title:"原中国饲料工业协会会长白美清一行莅临溢多利视察工作",

				des:"2009年，原中国饲料工业协会白美清会长一行莅临溢多利公司视察工作，对公司取得的成绩表示充分认可。",

				url:"<?php echo theme_asset('images/30/18.jpg'); ?>"

			},

			{

				title:"全国畜牧总站站长、中国饲料工业协会秘书长王宗礼莅临溢多利视察",

				des:"2020年11月，全国畜牧总站站长、中国饲料工业协会秘书长王宗礼一行莅临溢多利视察，对公司的高速发展及科研实力表示肯定。",

				url:"<?php echo theme_asset('images/30/19.jpg'); ?>"

			},

			{

				title:"原全国畜牧总站站长、中国饲料工业协会常务副会长兼秘书长李希荣等领导视察溢多利",

				des:"2016年，原全国畜牧总站站长、中国饲料工业协会常务副会长兼秘书长李希荣等领导一行莅临溢多利视察，对公司的发展态势表示肯定。",

				url:"<?php echo theme_asset('images/30/20.jpg'); ?>"

			},

			{

				title:"原中国畜牧兽医学会动物营养分会会长许振英教授来溢多利指导工作",

				des:"溢多利自1991年成立之初，董事长陈少美先生多次飞赴东北农业大学，与著名动物营养学家许振英教授进行深入的沟通交流，在推进企业的成长同时也推动了中国饲用酶制剂行业一步步朝前发展。",

				url:"<?php echo theme_asset('images/30/21.jpg'); ?>"

			},

			{

				title:"原中国畜牧兽医学会动物营养分会会长张子仪院士莅临溢多利并题词",

				des:"原中国畜牧兽医学会动物营养分会会长张子仪院士莅临溢多利并挥毫题词，寄托了对公司的美好祝福。",

				url:"<?php echo theme_asset('images/30/22.jpg'); ?>"

			},

			{

				title:"中国工程院李德发院士参观溢多利并指导工作",

				des:"中国工程院李德发院士作为我国动物营养与饲料科学领域的权威专家，到溢多利参观并指导工作。",

				url:"<?php echo theme_asset('images/30/23.jpg'); ?>"

			},

			{

				title:"麦康森院士莅临溢多利调研考察",

				des:"2018年，中国工程院麦康森院士莅临溢多利进行调研考察。",

				url:"<?php echo theme_asset('images/30/24.jpg'); ?>"

			},

			{

				title:"印遇龙院士在溢多利生物产业发展与行业高端论坛上作报告",

				des:"2016年，中国工程院印遇龙院士在溢多利生物产业发展与行业高端论坛上作报告。",

				url:"<?php echo theme_asset('images/30/25.jpg'); ?>"

			},

			{

				title:"溢多利荣获中国饲料工业协会“全国三十强饲料企业”称号",

				des:"2005年，溢多利荣获中国饲料工业协会颁发的“全国三十强饲料企业”称号，这是对公司实力的极大肯定。",

				url:"<?php echo theme_asset('images/30/26.jpg'); ?>"

			},

			{

				title:"陈少美董事长参加全国饲料工业协会三十强企业会议",

				des:"2005年，陈少美董事长参加全国饲料工业协会三十强企业会议并获奖。",

				url:"<?php echo theme_asset('images/30/27.jpg'); ?>"

			},

			{

				title:"陈少美董事长荣获“改革开放三十年推动饲料工业发展十大新锐人物",

				des:"2009年6月30日，农业部全国饲料工业办公室在北京隆重召开了“改革开放三十年饲料工业回顾与展望总结”，溢多利陈少美董事长荣获“改革开放三十年推动饲料工业发展十大新锐人物”。",

				url:"<?php echo theme_asset('images/30/28.jpg'); ?>"

			},

			{

				title:"溢多利荣获国家科技进步奖二等奖",

				des:"2014年12月，溢多利《饲料用酶制剂技术体系创新及重点产品创制》项目获得国家科学技术进步奖二等奖。据悉，中国农科院饲料研究所姚斌研究团队、溢多利研发中心李阳源博士等专家学者经过潜心研究，获得了具有自主知识产权和应用价值的新型饲料用酶基因百余个，创新了酶蛋白分子改良技术体系。",

				url:"<?php echo theme_asset('images/30/29.jpg'); ?>"

			},

			{

				title:"国家认定企业技术中心揭牌仪式",

				des:"2018年11月，溢多利国家认定企业技术中心举行揭牌仪式，公司董事长陈少美先生及珠海市政府领导揭牌。",

				url:"<?php echo theme_asset('images/30/30.jpg'); ?>"

			},

			{

				title:"溢多利子公司湖南鸿鹰生物科技有限公司“陈坚院士工作站”揭牌成立",

				des:"2018年，溢多利子公司湖南鸿鹰生物科技有限公司“陈坚院士工作站”揭牌成立，标志着溢多利在推进产学研合作方面迈出了新步伐，在推动科技创新方面取得了新成效。",

				url:"<?php echo theme_asset('images/30/31.jpg'); ?>"

			},

			{

				title:"溢多利举行股份改制创立大会",

				des:"2001年，溢多利完成股份制改造，为公司加速迈向资本市场奠定基础，公司发展进入了新的阶段。",

				url:"<?php echo theme_asset('images/30/32.jpg'); ?>"

			},

			{

				title:"湖南鸿鹰生物科技有限公司",

				des:"2014年，溢多利成功并购湖南鸿鹰生物科技有限公司，酶制剂产品全面进入饲料、能源、医药、食品、纺织、造纸等诸多工业领域。",

				url:"<?php echo theme_asset('images/30/33.jpg'); ?>"

			},

			{

				title:"河南利华制药有限公司",

				des:"2015年，溢多利成功并购河南利华制药有限公司，标志着公司正式进入生物医药产业。",

				url:"<?php echo theme_asset('images/30/34.jpg'); ?>"

			},

			{

				title:"湖南新合新生物医药有限公司",

				des:"2015年，溢多利并购湖南新合新生物医药有限公司，形成产业协同发展的新态势，步入跨越式发展快车道。",

				url:"<?php echo theme_asset('images/30/35.jpg'); ?>"

			},

			{

				title:"长沙世唯科技有限公司",

				des:"2018年，溢多利并购长沙世唯科技有限公司，正式进入药用植物提取领域。",

				url:"<?php echo theme_asset('images/30/36.jpg'); ?>"

			},

			{

				title:"溢多利资本化之路",

				des:"2014年1月28日上午9时25分，溢多利成功登陆深圳证券交易所，成为中国生物酶制剂行业首家上市企业。",

				url:"<?php echo theme_asset('images/30/37.jpg'); ?>"

			},

			{

				title:"溢多利总部大楼",

				des:"溢多利在珠海总部的统筹协调下，形成了生物酶制剂、生物医药、动物营养与健康三大产业协同发展的态势。",

				url:"<?php echo theme_asset('images/30/38.jpg'); ?>"

			},

			{

				title:"溢多利-内蒙古呼和浩特生产基地",

				des:"内蒙古溢多利生物科技有限公司是溢多利在内蒙古建立的北方酶制剂发酵生产基地，位于呼和浩特市托克托工业园。",

				url:"<?php echo theme_asset('images/30/39.jpg'); ?>"

			},

			{

				title:"溢多利湖南生物医药产业园",

				des:"溢多利在湖南津市投产“溢多利生物医药产业园”，为我国医药产业建设做出了更大的贡献。",

				url:"<?php echo theme_asset('images/30/40.jpg'); ?>"

			},

			{

				title:"溢多利-河南安阳生物医药生产基地",

				des:"河南利华制药有限公司是广东溢多利的全资子公司，生产和销售皮质激素原料药和核心中间体，产品远销全球各地。",

				url:"<?php echo theme_asset('images/30/41.jpg'); ?>"

			},

			{

				title:"溢多利-湖南浏阳动物营养与健康生产基地",

				des:"湖南美可达生物资源股份有限公司是溢多利旗下分子公司，主营天然植物源饲料添加剂等产品的研发、生产及销售。",

				url:"<?php echo theme_asset('images/30/42.jpg'); ?>"

			},

			{

				title:"生物酶制剂、动物营养与健康（珠海总部）生产基地",

				des:"广东溢多利生物科技股份有限公司珠海总部主要进行生物酶制剂、动物营养与健康产品的生产。",

				url:"<?php echo theme_asset('images/30/43.jpg'); ?>"

			},

			{

				title:"溢多利国家认定企业技术中心概况",

				des:"作为创新型生物技术企业，溢多利拥有雄厚的科研实力及创新研发平台优势。",

				url:"<?php echo theme_asset('images/30/44.jpg'); ?>"

			},

			{

				title:"溢多利国家认定企业技术中心团队合影",

				des:"溢多利在30年的高速发展中建立起一支高素质、多层次、结构合理的技术研发队伍。公司研发团队现有研发人员约400名，博士、硕士学历占比30%以上。",

				url:"<?php echo theme_asset('images/30/45.jpg'); ?>"

			},

			{

				title:"溢多利国家认定企业技术中心团队合影",

				des:"溢多利在30年的高速发展中建立起一支高素质、多层次、结构合理的技术研发队伍。公司研发团队现有研发人员约400名，博士、硕士学历占比30%以上。",

				url:"<?php echo theme_asset('images/30/45-1.jpg'); ?>"

			},

			{

				title:"溢多利研究院——生物酶制剂研究中心",

				des:"溢多利研究院生物酶制剂研究中心，主要进行饲料用酶制剂、食品用酶制剂、洗涤用酶制剂、纺织用酶制剂、能源用酶制剂、医药用酶制剂、造纸用酶制剂等产品的研发。",

				url:"<?php echo theme_asset('images/30/46.jpg'); ?>"

			},

			{

				title:"溢多利研究院——生物医药研究中心",

				des:"溢多利研究院生物医药研究中心，甾体激素生化合成技术世界领先，在国内率先形成从植物甾醇出发，开展甾体激素中间体、原料药、制剂全产业链技术的研发与产业化示范。",

				url:"<?php echo theme_asset('images/30/47.jpg'); ?>"

			},

			{

				title:"溢多利研究院——植物提取研究中心",

				des:"溢多利研究院植物提取研究中心，研究开发“博落回提取物原料药及其制剂博落回散”，成为我国首个二类新中兽药，“博落回散”成为首个二类中兽药类药物饲料添加剂；此后，研究开发“博普总碱原料药及其制剂博普总碱散”再次获批为国家二类新中兽药。",

				url:"<?php echo theme_asset('images/30/48.jpg'); ?>"

			},

			{

				title:"溢多利先进研发设备",

				des:"溢多利研发中心拥有国际先进的设备与一流的研发团队，极大地促进了公司科研实力的提升。",

				url:"<?php echo theme_asset('images/30/49.jpg'); ?>"

			},

			{

				title:"溢多利先进研发设备",

				des:"溢多利研发中心拥有国际先进的设备与一流的研发团队，极大地促进了公司科研实力的提升。",

				url:"<?php echo theme_asset('images/30/49-1.jpg'); ?>"

			},

			{

				title:"溢多利研发中心",

				des:"溢多利将重研发、促创新视为安身立命之根本，历经三十载耕耘，现已构建一套以溢多利研究院为核心的研发体系。",

				url:"<?php echo theme_asset('images/30/50.jpg'); ?>"

			},

			{

				title:"溢多利参与精准扶贫献爱心活动",

				des:"溢多利对口帮扶贫困家庭，参与精准扶贫献爱心活动。公司自成立以来，始终秉持奉献精神，传递社会大爱。",

				url:"<?php echo theme_asset('images/30/51.jpg'); ?>"

			},

			{

				title:"溢多利第一届奖学金、奖教金颁奖现场",

				des:"自1999年至今，溢多利在全国多所农业类高校设立“溢多利奖学金、奖教金”，为数千名莘莘学子提供了经济帮助。",

				url:"<?php echo theme_asset('images/30/52.jpg'); ?>"

			},

			{

				title:"溢多利第一届奖学金、奖教金颁奖现场",

				des:"自1999年至今，溢多利在全国多所农业类高校设立“溢多利奖学金、奖教金”，为数千名莘莘学子提供了经济帮助。",

				url:"<?php echo theme_asset('images/30/52-1.jpg'); ?>"

			},

			{

				title:"溢多利扶贫助学活动",

				des:"2006年5月31日，在珠海市红旗镇，溢多利举行扶贫助学活动。",

				url:"<?php echo theme_asset('images/30/53.jpg'); ?>"

			},

			{

				title:"溢多利人向汶川灾区人民奉献爱心",

				des:"2008年5月12日，四川汶川发生大地震，牵动着无数人的心。5月15日，全体溢多利人自发为灾区捐款。短短一小时，溢多利为汶川捐款达数十万元人民币。",

				url:"<?php echo theme_asset('images/30/54.jpg'); ?>"

			},

			{

				title:"情系玉树",

				des:"2010年4月14日，青海省玉树藏族自治州玉树县发生7.1级地震。15日，溢多利全体员工齐聚公司行政楼大厅进行捐款活动，陈少美董事长代表公司向玉树灾区捐款10万元人民币，珠海市有关领导全程参与。",

				url:"<?php echo theme_asset('images/30/55.jpg'); ?>"

			},

			{

				title:"营销团队冲刺业绩合影留念",

				des:"2014年2月9日，溢多利全体营销团队再接再厉、创造辉煌，冲刺业绩“开门红”。",
				url:"<?php echo theme_asset('images/30/56.jpg'); ?>"

			},

			{

				title:"20周年优秀员工表彰",

				des:"奋进数十载，缘聚溢多利。溢多利举办成立二十周年庆典暨总结表彰动员大会，在大会上表彰先进工作代表及优秀团队，同时勉励大家未来更加奋进。",

				url:"<?php echo theme_asset('images/30/57.jpg'); ?>"

			},

			{

				title:"团建——东澳岛拓展训练",

				des:"作为一家高度重视人才团队建设的企业，溢多利常态化举行公司营销动员活动及团队建设活动等，进一步加强了人才团队建设，增强了团队凝聚力，提升了全员客户服务意识，丰富了员工业余生活。",

				url:"<?php echo theme_asset('images/30/58.jpg'); ?>"

			}

			

		]
		var ahtml = '';
		var bhtml = '';
		for(var i=0;i<pic.length;i++){
			ahtml+=' <div class="swiper-slide "><div class="title"> <img  class="" src="'+pic[i].url+'" alt=""></div><div class="description"><span >'+pic[i].title+'</span>'+pic[i].des+'</div></div>';
			
		}
		$(".gallery-top .swiper-wrapper").html(ahtml);
		setTimeout(function(){
				var mySwiper = new Swiper('.gallery-top', {
				// loop: true,
				// autoplay: 6000,
				 //pagination: '.s',
				  autoHeight: true, //高度随内容变化
				paginationClickable :true,
				navigation: {
				  nextEl: ".swiper-button-next",
				  prevEl: ".swiper-button-prev"
				},
			})
			
		},1000)
	
		
		
		
        $(".video-detail .video-detail-close,#video-detail-con .video-close").on("click",function(){
            $(".video-detail").hide();
            $("#player").html("")
        })
		
		
        $(".page-news-video").on("click",function(){
            var vid = $(this).attr("vid");
            var des = $(this).attr("des");
            $("#player").html("")
            $(".video-detail").fadeIn(200)
            var width = document.getElementById("playerBox").scrollWidth;
            var height = width*0.5625; // 16/9 = 0.5625;
            var player = polyvPlayer({
                wrap: '#player',
                autoplay:false,
                'width':width,
                'height':height,
                'vid' : vid
            });
            $("#playerBox .des").text(des)
        })
	
		var screen_width = window.screen.width;
		 var screen_height = window.screen.height;    

		
	
			
			 //根据屏幕分辨率判断是否是手机
			 if(screen_width < 750 ){
					$(".gallery-lc").addClass("swiper-no-swiping")

			 }else{
				
				
			 }
			
			
		
		$(".fixed-nav").hide();
		
		$(".rcICon").on("click",function(){
			$(".rc-con").fadeIn(200)
		})
		$(".rc-close").on("click",function(){
			$(".rc-con").fadeOut(200)
		})
		
		$(".live-test-item").on("click",function(){
			var i = $(this).index(".live-test-item");
			
			$(this).addClass("active").siblings(".live-test-item").removeClass("active")
			$(".live .live-img-item ").eq(i).fadeIn(100).siblings(".live-img-item").hide()
		})
		
		
		var greetingsPlayerWidth =$("#greetingsPlayer").width();
        var greetingsPlayerHeight = greetingsPlayerWidth*0.5625; // 16/9 = 0.5625;
		
		var greetingsPlayer = polyvPlayer({
                wrap: '#greetingsPlayer',
                autoplay:false,
                'width':greetingsPlayerWidth,
                'height':greetingsPlayerHeight,
                'vid' : "ad514bee27a523f5e2fbce449dd6ce24_a"
		});
    })
</script>

