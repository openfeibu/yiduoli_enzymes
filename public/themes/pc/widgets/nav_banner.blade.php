<div class="pageBanner newsBanner" style=" background:#eee url({!!  '/image/original/'.$top_nav->image  !!}) no-repeat center / cover;">
    @if(Route::currentRouteName() == 'pc.industrial_enzyme')
    @else
    <div class="pageBanner-title wow fadeInUp animated" data-wow-duration=".6s" data-wow-delay=".3s"><div class="pageBanner-title-c"><span>{{ $top_nav->name }}{{ Route::currentRouteName() }}</span></div></div>
